...
namespace VEMC
{
    public class MapJob
    {
        ...
        public void Process()
        {
            ...
            if (list5 != null)
            {
                foreach (TmxObject tmxObject4 in list5)
                {
                    ...
                    {
                        using (List<TmxObjectPoint>.Enumerator enumerator3 = (List<TmxObjectPoint>.Enumerator)tmxObject4.Points.GetEnumerator())
                        {
                            while (enumerator3.MoveNext())
                            {
                                TmxObjectPoint tuple = enumerator3.Current;
                                coordsList.Add(new NbtInt((int)tuple.X));
                                coordsList.Add(new NbtInt((int)tuple.Y));
                            }
                            goto IL_88B;
                        }
                        goto IL_7FF;
                ...
                IL_7FF:
                    if (tmxObject4.ObjectType == TmxObjectType.Basic)
                    {
                        ...
                        coordsList.Add(new NbtInt(0));
                        coordsList.Add(new NbtInt((int)tmxObject4.Width));
                        coordsList.Add(new NbtInt(0));
                        coordsList.Add(new NbtInt((int)tmxObject4.Width));
                        coordsList.Add(new NbtInt((int)tmxObject4.Height));
                        coordsList.Add(new NbtInt(0));
                        coordsList.Add(new NbtInt((int)tmxObject4.Height));
                        goto IL_88B;
                    }
            ...
            if (list9 != null)
            {
                foreach (TmxObject tmxObject6 in list9)
                {
                    ...
                    {
                        using (List<TmxObjectPoint>.Enumerator enumerator3 = (List<TmxObjectPoint>.Enumerator)tmxObject6.Points.GetEnumerator())
                        {
                            while (enumerator3.MoveNext())
                            {
                                TmxObjectPoint tuple2 = enumerator3.Current;
                                list10.Add(new NbtInt((int)tmxObject6.X + (int)tuple2.X));
                                list10.Add(new NbtInt((int)tmxObject6.Y + (int)tuple2.Y));
                                num9++;
                            }
                            goto IL_E84;
                        }
            ...
            if (list12 != null)
            {
                foreach (TmxObject tmxObject8 in list12)
                {
                    ...
                    if (text4 != null)
                    {
                        ...
                        {
                            using (List<TmxObjectPoint>.Enumerator enumerator3 = (List<TmxObjectPoint>.Enumerator)tmxObject8.Points.GetEnumerator())
                            {
                                while (enumerator3.MoveNext())
                                {
                                    TmxObjectPoint tuple3 = enumerator3.Current;
                                    list14.Add(new NbtInt((int)tuple3.X));
                                    list14.Add(new NbtInt((int)tuple3.Y));
                                    num13++;
                                }
                                goto IL_1191;
                            }
            ...
            if (list15 != null)
            {
                foreach (TmxObject tmxObject9 in list15)
                {
                    ...
                    Console.WriteLine($"3");
                    nbtCompound4.Add(new NbtInt("x", (int)tmxObject9.X));
                    nbtCompound4.Add(new NbtInt("y", (int)tmxObject9.Y));
                    nbtCompound4.Add(new NbtInt("w", (int)tmxObject9.Width));
                    nbtCompound4.Add(new NbtInt("h", (int)tmxObject9.Height));
            ...
            if (list18 != null)
            {
                foreach (TmxObject tmxObject10 in list18)
                {
                    ...
                    mapPart20.Add(new NbtFloat("x", (int)tmxObject10.X));
                    mapPart20.Add(new NbtFloat("y", (int)tmxObject10.Y));
                    mapPart20.Add(new NbtFloat("w", (int)tmxObject10.Width));
                    mapPart20.Add(new NbtFloat("h", (int)tmxObject10.Height));
                    num16++;
                }
            }
        ...
        public List<List<IntPoint>> BuildMesh()
        {
            ...
            int num = width * height * map.TileLayers.Count;
            int num2 = 0;
            MeshBuilder meshBuilder = new MeshBuilder();
            string text = null;
            for (int i = map.TileLayers.Count - 1; i >= 0; i--)
            {
                string name = map.TileLayers[i].Name;
                int count = map.TileLayers[i].Tiles.Count;
                for (int j = 0; j < count; j++)
                {
                    TmxLayerTile tmxLayerTile = map.TileLayers[i].Tiles[j];
